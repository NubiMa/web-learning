...
@section('content')
    <div class="mb-6 flex flex-col md:flex-row items-center justify-between gap-4">
        ...
        <div class="relative w-full md:w-96">
            <input type="text" placeholder="Search quizzes..." data-admin-search="table"
                ...
                @foreach ($quizzes as $quiz)
                    <tr
                        ...
                        <td class="px-4 py-3 md:px-6 md:py-4 block md:table-cell">
                            ...
                            <div class="text-xs md:text-sm text-gray-500 line-clamp-1">{{ $quiz->description }}</div>
                        </td>
                        <td class="px-4 py-2 md:px-6 md:py-4 block md:table-cell">
                            ...
                            <span
                                class="px-2 py-1 text-xs rounded-full bg-blue-100 text-blue-800">{{ $quiz->module->title }}</span>
                        </td>
                        <td class="px-4 py-2 md:px-6 md:py-4 block md:table-cell">
                            <span
                                class="md:hidden text-xs font-semibold text-gray-500 uppercase tracking-wider mr-2">Questions:</span>
                            <span class="text-sm text-gray-900">{{ $quiz->questions->count() }} questions</span>
                        </td>
                        <td class="px-4 py-2 md:px-6 md:py-4 block md:table-cell">
                            <span
                                class="md:hidden text-xs font-semibold text-gray-500 uppercase tracking-wider mr-2">Passing
                                Score:</span>
                            <span class="text-sm text-gray-900">{{ $quiz->passing_score }}%</span>
                        </td>
                        <td class="px-4 py-2 md:px-6 md:py-4 block md:table-cell">
                            <div class="flex items-center justify-between md:justify-start">
                                ...
                                <form action="{{ route('admin.quizzes.toggle', $quiz) }}" method="POST">
                                    @csrf
                                    <button type="submit"
                                        class="px-3 py-1 rounded-full text-xs font-semibold {{ $quiz->is_active ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800' }}">
                        ...
                        <td
                            ...
                            <div class="flex items-center justify-end space-x-3">
                                <a href="{{ route('admin.quizzes.show', $quiz) }}"
                                    class="text-blue-600 hover:text-blue-900 text-sm font-medium">Manage</a>
